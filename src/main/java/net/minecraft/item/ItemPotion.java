...
public class ItemPotion extends Item {
    ...
    public int getColorFromItemStack(ItemStack stack, int renderPass) {
        ...
        // Pour les potions custom, utiliser la couleur du premier effet
        if (stack.hasTagCompound() && stack.getTagCompound().hasKey("CustomPotionEffects", 9)) {
        ...
        return this.getColorFromDamage(stack.getMetadata());
    }
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        // Potions custom : utiliser le nom du tag display si présent
    ...
    public void getSubItems(Item itemIn, CreativeTabs tab, List<ItemStack> subItems) {
        ...
        // ── Potions custom : Fall Protection et Haste ──────────────────────────
        subItems.add(createCustomPotion(itemIn, Potion.fallProtection.id, 0, 3600, "Potion of Fall Protection"));
        subItems.add(createCustomPotion(itemIn, Potion.fallProtection.id, 1, 1800, "Potion of Fall Protection II"));
        subItems.add(createCustomPotion(itemIn, Potion.fallProtection.id, 0, 3600, "Splash Potion of Fall Protection", true));
        subItems.add(createCustomPotion(itemIn, Potion.fallProtection.id, 1, 1800, "Splash Potion of Fall Protection II", true));
        subItems.add(createCustomPotion(itemIn, Potion.digSpeed.id, 0, 3600, "Potion of Haste"));
        subItems.add(createCustomPotion(itemIn, Potion.digSpeed.id, 1, 1800, "Potion of Haste II"));
        subItems.add(createCustomPotion(itemIn, Potion.digSpeed.id, 0, 3600, "Splash Potion of Haste", true));
        subItems.add(createCustomPotion(itemIn, Potion.digSpeed.id, 1, 1800, "Splash Potion of Haste II", true));
    }

    /**
     * Crée un ItemStack de potion custom avec effets NBT pour le creative tab.
     */
    public static ItemStack createCustomPotion(Item itemIn, int effectId, int amplifier, int duration, String displayName) {
        return createCustomPotion(itemIn, effectId, amplifier, duration, displayName, false);
    }

    public static ItemStack createCustomPotion(Item itemIn, int effectId, int amplifier, int duration, String displayName, boolean splash) {
        // meta 8206 = base potion, 8206|16384 = splash
        int meta = splash ? (8206 | 16384) : 8206;
        ItemStack stack = new ItemStack(itemIn, 1, meta);
        NBTTagCompound tag = new NBTTagCompound();

        NBTTagList effects = new NBTTagList();
        NBTTagCompound eff = new NBTTagCompound();
        eff.setByte("Id", (byte) effectId);
        eff.setByte("Amplifier", (byte) amplifier);
        eff.setInteger("Duration", duration);
        eff.setByte("ShowParticles", (byte) 1);
        effects.appendTag(eff);
        tag.setTag("CustomPotionEffects", effects);

        NBTTagCompound display = new NBTTagCompound();
        display.setString("Name", "\u00a7f" + displayName);
