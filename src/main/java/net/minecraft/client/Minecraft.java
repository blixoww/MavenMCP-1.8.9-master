...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private void startGame() throws LWJGLException, IOException
    {
        ...
        // ── Initialisation du système de packets custom (Plugin Messaging) ──
    ...
    public void runTick() throws IOException
    {
        ...
        // Tick des macros
        net.minecraft.client.macro.MacroManager.INSTANCE.onTick(this);
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (this.gameSettings.keyBindDrop.isPressed())
            {
                if (!this.thePlayer.isSpectator())
                {
                    // Vérifier si l'item en main est un item custom (id 432-470)
                    net.minecraft.item.ItemStack heldItem = this.thePlayer.getHeldItem();
                    if (heldItem != null)
                    {
                        int heldId = net.minecraft.item.Item.getIdFromItem(heldItem.getItem());
                        if (heldId >= 432 && heldId <= 470)
                        {
                            // Item custom : envoyer un packet au serveur pour le drop
                            boolean dropAll = GuiScreen.isCtrlKeyDown();
                            int dropAmount = dropAll ? heldItem.stackSize : 1;
                            net.minecraft.item.ItemStack toDrop = heldItem.copy();
                            toDrop.stackSize = dropAmount;
                            // Retirer de l'inventaire local
                            if (dropAll) {
                                this.thePlayer.inventory.setInventorySlotContents(this.thePlayer.inventory.currentItem, null);
                            } else {
                                heldItem.stackSize--;
                            ...
                            // Envoyer le packet custom drop au serveur
                            try {
                                net.minecraft.network.PacketBuffer buf = net.minecraft.client.custompackets.PacketSender.newBuffer();
                                buf.writeVarIntToBuffer(net.minecraft.client.custompackets.PacketId.CUSTOM_ITEM_DROP);
                                buf.writeItemStackToBuffer(toDrop);
                                net.minecraft.client.custompackets.PacketSender.send(
                                    net.minecraft.client.custompackets.PacketChannel.CLIENT_TO_SERVER, buf);
                            } catch (Exception e) {
                                // Fallback vanilla
                                this.thePlayer.dropOneItem(dropAll);
                            }
                        }
                        else
                        {
