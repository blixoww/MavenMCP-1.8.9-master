...
import net.minecraft.client.gui.ui.KeyStrokeWidget;
...
public class GuiIngame extends Gui {
        ...
        try {
            ...
            try {
                ...
                if (!cfg.exists()) {
                    ...
                    ks.setEnabled(true);
                    ks.setRGBMode(true);
                    for (int i = 0; i < 7; i++) ks.setProp("showKey" + i, Boolean.TRUE); // 0-6 : Z,Q,S,D,LMB,RMB,Espace
                    ks.setPosition(Math.max(10, sw - 80 - 10), 10);
                }
            } catch (Throwable ignored) {
            }
            uiManager.register(ks);
            // ensure saved config is applied to keystrokes if present
            try {
                uiManager.loadConfig();
                java.io.File cfg = new java.io.File(Minecraft.getMinecraft().mcDataDir, "config/ui_widgets.json");
                if (!cfg.exists()) uiManager.saveConfig();
            } catch (Throwable ignored) {
            }
        } catch (Exception e) {
        }
    ...
    public void updateToggleKeys() {
        ...
        // Toggle Sneak : si la fonctionnalité est activée dans les options,
        // la touche Sneak classique bascule entre accroupi/non-accroupi
        if (gs.toggleSneakEnabled) {
            boolean sneakDown = GameSettings.isKeyDown(gs.keyBindSneak);
            if (sneakDown && !prevToggleSneakDown) {
                gs.isToggleSneakActive = !gs.isToggleSneakActive;
            }
            prevToggleSneakDown = sneakDown;
        } else {
            // Mode normal : on reset l'état toggle et on laisse la touche gérer normalement
            gs.isToggleSneakActive = false;
            prevToggleSneakDown = false;
        }

        // Toggle Sprint : si la fonctionnalité est activée dans les options,
        // la touche Sprint classique bascule entre sprint/non-sprint
        if (gs.toggleSprintEnabled) {
            boolean sprintDown = GameSettings.isKeyDown(gs.keyBindSprint);
            if (sprintDown && !prevToggleSprintDown) {
        ...
        gs.isZoomActive = GameSettings.isKeyDown(gs.keyBindZoom);
    }
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        // If scaled resolution changed since last frame, update widget alignment
        if (i != lastScaledWidth || j != lastScaledHeight) {
            lastScaledWidth = i;
            lastScaledHeight = j;
            try {
                uiManager.updateAllWidgetAlignment();
            } catch (Throwable ignored) {
            }
        }
    ...
    // Correction : pas d’@Override ni de super.onResize
    public void onResize(Minecraft mcIn, int w, int h) {
        try {
            net.minecraft.client.gui.ui.UIManager.getInstance().updateAllWidgetAlignment();
